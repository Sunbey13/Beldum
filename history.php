  ...
  <div class="container">
    <form action="history.php" method="GET">
      <div class="row">
          ...
          <div class="col-md-2">
          <h4 class="non-print">Выберите дату:</h4>
          <h4 class="put-list">Отчет рейсов из истории за:</h4>
          </div>
          <div class="col-md-3">
            <input type="date" class="form-control print-date" name="Date" value="<?php $print_date = $_GET['Date']; echo $print_date; ?>">
          </div>
          <div class="col-md-1">
            <input type="submit" name="del" value="Показать" class=" btn-history btn btn-primary">
          </div>
      </div>
    </form>
    ...
    $sql = "SELECT `reis`.`id`, `reis`.`Date`, `voditel`.`FIO`, `organ_adres`.`Address`, `organ`.`Name_org` FROM voditel 
        ...
        WHERE `Del` = 0 AND `reis`.`Date` = '$Date'";
